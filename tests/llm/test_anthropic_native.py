...
def test_tool_use_json_path_forces_tool_with_schema():
    ...
    msg = _Message([_Block(type="tool_use", name="emit_result", input={"title": "Chapter 1"})], _Usage(5, 5))
    p, client = provider(msg)

    resp = p.generate(LLMRequest(prompt="make a chapter", response_format="json_schema", json_schema=schema))
