...
from pipeline.providers.errors import (
...
def test_validate_allows_member_and_defaults_model():
    assert validate_selection("openai", allowed={"openai", "anthropic"}) == ("openai", "gpt-4o-mini")
...
def test_factory_builds_when_allowed():
    p = build_provider("openai", api_key="k", allowed={"openai", "deepseek"}, http_client=_mock_client())
