...
| **6 — Launch hardening** | ✅ **BUILT** (2026-06-30). **Spend ceiling (O7)** — hard per-account `MANAGED_ACCOUNT_SPEND_CEILING_MICROS` backstop in `access.over_cap`, bounds OUR spend even on an unlimited plan/staff; **anomaly alarm** (`managed_spend_alarm` at a warn fraction); **multi-provider** vault + pricing (OpenAI/Groq/Gemini, ToS-cleared — Gemini paid-only); **margin data** `GET /api/v1/admin/billing/usage-summary`; **rotation runbook** `docs/MANAGED_KEY_ROTATION.md`. | A margin *UI* dashboard on top of the summary endpoint is a later nicety. |
