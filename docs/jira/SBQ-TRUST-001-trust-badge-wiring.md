...
> mobile half below is **implemented**; the backend packager remains blocked on
> `wegofwd-llm` v0.2.0.
...
1. Ship `wegofwd-llm` **v0.2.0** (ADR-015 code: `trust.py` + schema + tests). *(drafted)*
2. Bump `backend/requirements.txt`: `wegofwd-llm[anthropic] @ …@v0.2.0`.
3. Apply backend diffs (1–3) + mobile wiring (4–5).
